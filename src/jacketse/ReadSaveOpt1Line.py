...
def ReadOptFile(optfile,opttitlines=3,opthdrlines=1,optuntlines=1):
    ...
    return desvars
#__________________________________________________________#
def ReadTab1Line(casefile,caseno,desvarnames,titlines=3,hdrlines=1,untlines=1):
    ...
    INPUTS \n
        casefile - rstring, complete 'r'+path+name to text file containing cases to run. Format of file is to be found at RunAllJacketTwrs.py.\n
        caseno   - int, line to read out of the file, corresponding to case number.\n
        desvarnames - list of official names of design variables in teh exact and fixed order, usually got from an ordered dictionary from DesVars class.\n
    OPTIONALS:
    ...
    Desprms.RNA_F=np.array([float(line[11]),0.,0.,float(line[12]),0.,0.])
    Desprms.RNAins.mass=float(line[10])
    Desprms.RNAins.CMzoff=float(line[13])
    Desprms.RNAins.Thzoff=float(line[14])
    Desprms.f0=float(line[15])
    Desprms.legndiv=int(line[17])
    Desprms.nbays=int(line[18])
    Desprms.mxftprint=float(line[19])

    #Assign bounds
    offset=19 #this is an attempt to miniize issues if the table changes
    idx=(np.arange(1,37)+offset)
    idx=idx[::2]
...
def SaveOpt1Line(outdir,caseno,casename,desvars,rescobyla,myjckt,xlsfilename,Desprms,xlsfile_only=False):
    ...
    if (isinstance(xlsfilename,str) and (os.path.exists(xlsfilename) or  os.access(os.path.dirname(xlsfilename), os.W_OK))):
        ...
        sheet.write(turbrow+6,1,'RNA CMzoff'); sheet.write(turbrow+6,2,Desprms.RNAins.CMzoff) ;sheet.write(turbrow+6,3,'[m]')
        sheet.write(turbrow+7,1,'RNA Thzoff'); sheet.write(turbrow+7,2,Desprms.RNAins.Thzoff) ;sheet.write(turbrow+7,3,'[m]')
        ...
        sheet.write(rowno+19,2,(myjckt.Frameouts2.mass[0]-myjckt.Tower.Twrouts.mass-myjckt.TP.TPouts.mass)/1.e3)  #mass of main lattice no TP
        #units
        ...
        sheet.write(tprow+13,2,myjckt.TPinputs.hstem[0]+0.05+0.2)  #added the deck stringer height
        ...
        sheet.write(Totrow,2,(myjckt.TP.TPouts.TPlumpedMass[0]+myjckt.Frameouts2.mass[0]+myjckt.Mpiles)/1.e3,\
        ...
        #values
        sheet.write(Freqrow+1,2,myjckt.Frameouts2.Freqs[0], easyxf('font: name Arial, bold True'))
        sheet.write(Freqrow+2,2,myjckt.Frameouts2.Freqs[1], easyxf('font: name Arial, bold True'))
        #units
