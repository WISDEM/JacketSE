...
import ww_loads as ww    #Possibly to replace with Andrew's stuff
#import ApiCodeChecks as API
import JTwrUtilization as TwrUtil #Possibly to replace with Andrew's stuff
...
##    # inputs
##    wdepth   = Float( units='m', desc='Water Depth')   #TO DO, this should really come from environment
##    wlevel   = Float( units='m', desc='Water Level: Distance from bottom of structure (including buried pile) to water surface')   #TO DO, this should really come from environment
##    T        =Float(  units='s',desc='50-yr wave period (single wave here, not peak spectral period)')
##    HW       =Float(  units='m',desc='50-yr wave height (half-amplitude of sinusoid)')
##    psi      =Float(  units='deg',desc='Wave direction angle relative to downwind')
##    rho      =Float(1027.,  units='kg*m**-3',desc='Water Density')
##    Cd       =Float(1.25,   units=None,desc='Drag Coefficient')
...
##    # inputs
##    al_shear =Float(0.14,  units=None,      desc='ower law exponent wind from IEC 61400-3')
##    psi      =Float(       units='deg',     desc='Wind angle relative to downwind. positive according to RHR with positive z direction')
##    rho      =Float(1.225, units='kg*m**-3',desc='Air Density. Set to 0 if no tower wind load is included (thrust from rotor still on)')
##    U50HH   = Float(70.,   units='m/s',     desc='50-yr return 3-s gust [m/s] :From IEC Class I')   #TO DO, this should really come from environment
##    HH       =Float(       units='m',       desc='Hub-height above MSL')
##    Cd       =Float(1.25,  units=None,      desc='Drag Coefficient for tower in air flow')
##
...
##    twr_drag= Array(dtype=np.float, units='N', desc='Drag forces on tower nodes')
##    nds_frc=  Array(dtype=np.float,            desc='Concentrated Forces and Moments: (NODE ID, Fx,Fy,Fz,Mxx,Myy,Mzz)')
###______________________________________________________________________________#
##class JcktLoad(Component):
...
##    windIns     =VarTree(WindInputs(),     iotype='in', desc='Wind Data')
##    waterIns    =VarTree(WaterInputs(),    iotype='in', desc='Water Data')
...
##    twrDs       =Array(dtype=np.float, units='m',     iotype='in',desc='ODs of Tower.')
##
...
##    wind_dict=  Dict(                  iotype='out', desc='Dictionary containing wind properties to be used by utilization component')
##    water_dict= Dict(                  iotype='out', desc='Dictionary containing water properties to be used by utilization component')
##    twr_wndload=Dict(                  iotype='out', desc='Dictionary containing wind loading data on the tower alone to be used by utilization component')
##    twr_wtrload=Dict(                  iotype='out', desc='Dictionary containing water loading data on the tower alone to be used by utilization component (placeholder till we make tripods)')
##
##    def execute(self):
##        #Simplify nomenclature
##        nlegs=self.JcktGeoIn.nlegs
##        batter=self.JcktGeoIn.batter
##        wdepth=self.waterIns.wdepth
##        VPFlag=self.VPFlag
##        al_bat3D=self.al_bat3D
##        CMzoff=self.CMzoff
##        TwrRigidTop=self.TwrRigidTop
##
##        #Make two dictionaries for consistency with this loading routine but to be revised later
##        water_dict={'T':self.waterIns.T, 'hw':self.waterIns.HW, 'rho':self.waterIns.rho,\
##                     'Cd':self.waterIns.Cd, 'Cm':self.waterIns.Cm,'psi':np.deg2rad(self.waterIns.psi),\
##                     'wdpth':self.waterIns.wdepth, 'wlevel':self.waterIns.wlevel}
##
##        wind_dict={'al_shear':self.windIns.al_shear, 'HH':self.windIns.HH, 'rho':self.windIns.rho,\
##                     'Cd':self.windIns.Cd, 'psi':np.deg2rad(self.windIns.psi),'U50HH':self.windIns.U50HH}
##
##        #get leg and tower indices wihtin JcktGeoOut.nodes
...
##        pileZs=np.array([])
##        pileDs=np.array([])
##        pilendIDs=np.array([]).reshape([-1,1])
##        if Pilemems.size:
##            pileidx=np.arange(Pilemems[0,0],Pilemems[-1,1]/nlegs+1)-1   #indices of pile 1
##            pilendIDs=np.arange(Pilemems[0,0],Pilemems[-1,1]+1).reshape([-1,1])  #actual node IDs for all pile nodes to be used later
##            pileZs=self.JcktGeoOut.nodes[pileidx,2]
##            pileDs=self.pillegDs[0]
##
##        #legs and Tower must exist
##        legidx=np.arange(Legmems[0,0],Legmems[-1,1]/nlegs+1)-1 #indices of leg 1
##        legndIDs=np.arange(Legmems[0,0],Legmems[-1,1]+1).reshape([-1,1]) #actual node IDs of legs
##        twridx=np.arange(Twrmems[0,0],Twrmems[-1,1]+1)-1 #Indices of tower nodes
##        twrndIDs=np.arange(Twrmems[0,0],Twrmems[-1,1]+1).reshape([-1,1]) #Actual IDs of tower nodes, possibly including RNA node in RigidTop=True
##
##        #Zs and OD of pile, leg, tower
##        legZs=self.JcktGeoOut.nodes[legidx,2]
##        legDs=np.hstack((self.pillegDs[legidx[0]:],self.pillegDs[-1]))#ODs are associated with members not nodes, so I need to replicate for the last (top) node
##        twrZs=self.JcktGeoOut.nodes[twridx,2]
##        twrDs=np.hstack((self.twrDs,self.twrDs[-1])) #ODs are associated with flexible members not nodes, so I need to replicate for the last (top) node;
##                                                     #this should be conservative as Dt may be a little less than D[-1]
##
##        #COSINE MATRIX FROM LOCAL TO GLOBAL COORDINATE SYSTEMS
##        DIRCOSwind=np.array([ [np.cos(wind_dict['psi']) , -np.sin(wind_dict['psi']) ,0.],
##                          [np.sin(wind_dict['psi']) , np.cos(wind_dict['psi']),0.],
##                          [0.,                         0.,                      1.]])
##        DIRCOSwave=np.array([ [np.cos(water_dict['psi']) , -np.sin(water_dict['psi']) ,0.],
##                          [np.sin(water_dict['psi']) , np.cos(water_dict['psi']),0.],
##                          [0.,                         0.,                      1.]])
##
##
##        #Call ww_loads first and get the values of the loads at the nodes of pile and leg 1
##        #for the other legs, the wdrag is going to be the same
##
##        if nlegs== 4: #Diagonal loading condition ONLY for the time being
##
##            pillegZs=np.hstack((pileZs,legZs))
##            pillegDs=np.hstack((pileDs,legDs))
##            wind_load,water_load=ww.ww_loads(wind_dict,water_dict,pillegZs,pillegDs,self.gravity)
##
##            wl_idx=np.nonzero(water_load['drag'])[0] #indices of the leg nodes where water loading is applied
##
...
##
##            wDrag=water_load['drag']*deltaz #[N] forces
##
...
##            if VPFlag:  #Need to account for hte cos
##               #Forces on legs 1 (and 3 though sign of Fz reversed)
##                wDrag1_3[pileidx]=wDrag2_4[pileidx]
##
##            n_wloadsleg=wl_idx.size
##            n_wloads=n_wloadsleg*nlegs #Total number of wave load nodes
##
##            wave_ndsfrc=np.zeros([n_wloads,7])
...
##                nd_ids=np.vstack((pilendIDs[idx0:idx1],legndIDs[idx2:idx3]))[wl_idx]
##                if np.mod(ii,2)==0:
##                    lds=wDrag2_4[wl_idx,:]
##                elif ii==1:
...
##                    lds=wDrag1_3[wl_idx,:]
##
##                idx0=n_wloadsleg*ii #start index in wave_ndsfrc
##                idx1=idx0+n_wloadsleg  #stop index in wave_ndsfrc
##                wave_ndsfrc[idx0:idx1,:-3]=np.hstack((nd_ids,lds))
##
##                #Store wave loads
...
##
##
##        #Now Call ww_loads and get the values of the loads at the nodes of Tower
##
##        if twrZs.size>twrDs.size:#this may include a fictitious z for the attachment to RNA (CMzoff)
##            twrZs=twrZs[0:-1]  #pop the last element
##
...
##        self.twr_wndload,self.twr_wtrload=ww.ww_loads(wind_dict,water_dict,twrZs,twrDs) #just wind loading but store wtrload perhaps in the future for tripods
##        junk=(self.twr_wndload['drag']*deltaz).reshape([-1,1]) #[N] forces
##        #decompose along local x,y and add z component
##        junk=np.hstack((junk,np.zeros([junk.size,2]))).T
##        self.Loadouts.twr_drag=np.dot(DIRCOSwind , junk)    #*np.sin(wind_dict['psi']), junk*0.])
##
...
##        n_twrlds=(twrndIDs.shape[0]-TwrRigidTop)*(self.Loadouts.twr_drag.any()) #distributed loads along tower+RNA node load
##        twr_ndsfrc=np.zeros([n_twrlds,7])
##        twr_ndsfrc[0:n_twrlds,:-3]=np.hstack((twrndIDs[0:len(twrndIDs)-TwrRigidTop].reshape([-1,1]),self.Loadouts.twr_drag.T))
##
##        #--------------Add RNA concentrated forces----------------#
##        RNAload=np.dot(DIRCOSwind,self.RNA_F.reshape([2,3]).T) #Gravity is already accounted for by Frame3DD for concentrated masses: Jacket.Twr.RNAmass*aux['g_z'],np.zeros(3)))
##
##        # If we apply this load at tower-top and not tower-top+CMzoff we need to account for different DeltaZ
##        if  not(TwrRigidTop):
##            RNAload[0,1] -= RNAload[1,0]*CMzoff #Mxx
...
##        self.Loadouts.nds_frc=np.vstack((wave_ndsfrc, twr_ndsfrc))  #Concentrated loads
##
##        #Also store the water and wind dictionaries for later
##        self.wind_dict=wind_dict
##        self.water_dict=water_dict
#______________________________________________________________________________#
#                                UTILIZATION                                   #
#______________________________________________________________________________#
##class UtilOutputs(VariableTree):
##    """Jacket Utilization Basic Outputs"""
##    GLUtil=   Array(dtype=np.float, units=None, desc='Tower Utilization at each given section (global buckling)')
##    EUshUtil= Array(dtype=np.float, units=None, desc='Tower Utilization at each given section (shell buckling)')
##
##    t_util=   Array(dtype=np.float, units=None, desc='Member Utilization (tensile strength)')
##    cb_util=  Array(dtype=np.float, units=None, desc='Member Utilization (compression-bending buckling)')
##    XjntUtil=  Array(dtype=np.float,units=None, desc='X-Joint Utilization')
##    KjntUtil=  Array(dtype=np.float,units=None, desc='K-Joint Utilization')
##
##    EmbedLp=  Float(units='m',  desc='Embedded pile properties: length, and mass (for nlegs piles)')
##    EmbedMass=Float(units='kg', desc='Embedded pile properties: mass (for nlegs piles)')
###______________________________________________________________________________#
##class JcktUtil(Component):
...
##
##    wind_dict=  Dict(                 iotype='in',desc='Dictionary containing wind properties as calculated by JcktLoad')
##    water_dict= Dict(                 iotype='in',desc='Dictionary containing water properties as calculated by JcktLoad')
##    twr_wndload=Dict(                 iotype='in', desc='Dictionary containing wind loading data on the tower alone as calculated by JcktLoad')
##    twr_wtrload=Dict(                 iotype='in', desc='Dictionary containing water loading data on the tower alone as calculated by JcktLoad (placeholder till we make tripods)')
##
##    gravity     =Float(                units='m/s**2',iotype='in',desc='Acceleration Gravity.')
##
##    #outputs
##    Utilouts=   VarTree(UtilOutputs(), iotype='out', desc='Jacket Utilization Basic Outputs')
##
##    def execute(self):
##        #Simplify nomenclature
##        nlegs=self.nlegs
...
##
##        idx0_tower=Twrmems[0,0]  #Tower start index
##
...
##        mbr_idx2.sort()
##
##        XjntChk,XjntUtil=API.XjntChk(self.XjntIDs,self.JcktGeoOut.mems,MbrFrcs[mbr_idx2,:],mbr_strct)
##        max_XjntUtil=np.nanmax(XjntUtil)
##        #Lets us check the K-joints now
##        KjntChk,KjntUtil=API.KjntChk(self.KjntIDs,self.JcktGeoOut.mems,MbrFrcs[mbr_idx2,:],mbr_strct)
##        max_KjntUtil=np.nanmax(KjntUtil)
##
##        #__________________________________________#
##        #Tower Utilization
##
##        #Call JTwrUtilization to check tower util
##        Twr=self.Twrouts.TwrObj
##        Twr.Dt=self.Dt
##        Twr.tt=self.tt
...
#______________________________________________________________________________#
class RunFrame3DDmodal(Component):
    """This component runs Frame3DD for the jacket-tower assembly in modal analysis mode.
    """

    #inputs
    SPI_Kmat =Array(np.array([]),dtype=np.float,iotype='in', desc='Equivalent Stiffness Matrix at the pile head as calculated by pile SoilPileStiffness')
    clamped  =Bool( iotype='in', desc='This flag indicates that the pile or leg bottom will be clamped. Also if AF is used this must be set to true.')
    JcktGeoOut = VarTree(JcktGeoOutputs(), iotype='in', desc='Geometry of the Jacket -Node Coordinates and Member Connectivity')

    FrameAuxIn = VarTree(Frame3DDaux(),    iotype='in', desc='Auxiliary Data for Frame3DD')
    Frameouts=VarTree(Frame3DDOutputs(), iotype='in', desc='Basic output data from Frame3DDstatic')
    #outputs
    Frameouts2=VarTree(Frame3DDOutputs(), iotype='out', desc='Basic output data from Frame3DD')

    def execute(self):
        self.Frameouts2=self.Frameouts #Point to the original one
        if not(self.clamped):  #RUN THIS COMPONENT ONLY IF THERE IS A NEED TO DO IT, I.E. NOT CLMAPED SITUATION
            nodes = NodeData(np.arange(1,self.JcktGeoOut.nNodesJckt+1), self.JcktGeoOut.nodes[:,0],self.JcktGeoOut.nodes[:,1], self.JcktGeoOut.nodes[:,2], self.JcktGeoOut.radii)
            ...
            load = StaticLoadCase(self.FrameAuxIn.gvector[0], self.FrameAuxIn.gvector[1], self.FrameAuxIn.gvector[2]) #gravity load to be added no matter what, so that frame3dd does not complain
            frame.addLoadCase(load)
            #Add modal analysis here
            addGravityLoad=True #This means it will add self-weight of extra point masses
            ...
            frame.enableDynamics(self.FrameAuxIn.nModes, self.FrameAuxIn.Mmethod, self.FrameAuxIn.lump, self.FrameAuxIn.tol, self.FrameAuxIn.shift)
                                                #joint ID,                    mass,                              IXX
            frame.changeExtraNodeMass(self.JcktGeoOut.jnt_masses[:,0], self.JcktGeoOut.jnt_masses[:,1], self.JcktGeoOut.jnt_masses[:,2],\
                                                #IXX,                         IYY                                 #IXY
                                       self.JcktGeoOut.jnt_masses[:,3] , self.JcktGeoOut.jnt_masses[:,4], self.JcktGeoOut.jnt_masses[:,5], \
                                               #IXZ                           IYZ                         CMxoff
                                       self.JcktGeoOut.jnt_masses[:,6],  self.JcktGeoOut.jnt_masses[:,7], self.JcktGeoOut.jnt_masses[:,8], \
                                               #CMyoff               CMzoff
                                       self.JcktGeoOut.jnt_masses[:,9],  self.JcktGeoOut.jnt_masses[:,10],addGravityLoad)

            displacements, forces, reactions, internalForces, mass, modal = frame.run()
            #self.Frameouts.mass=np.array([mass.struct_mass,mass.total_mass])

            self.Frameouts2.Freqs=modal.freq[0:2]  #First 2 eiegenfreqs
...
class JacketAsmly(Assembly):
    ...
    def configure(self):
        ...
        if clamped:  #TO CHECK IF THIS WORKS or if we need afflag
        ...
        #outs
        #self.connect('Build.JcktGeoOut','JcktGeoOut')
        ...
        if self.JcktGeoIn.clamped:  #TO CHECK IF THIS WORKS or if we need afflag
